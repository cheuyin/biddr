...
export const PostComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({
        message: `${email} successfully created new comment on post: ${postId}`,
      });
  } catch (err) {
    return res.send(err.toString());
  }
...
export const PutComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: `Successfully updated comment ${commentId}` });
  } catch (err) {
    return res.send(err.toString());
  }
};
...
export const DeleteComment = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: `Successfully deleted comment ${commentId}` });
  } catch (err) {
    return res.send(err.toString());
  }
};
