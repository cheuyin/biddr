...
import {
  ...
  QueryHighestBidForAuction,
  CreateBid,
} from "../services/TransactionTable";
...
export const GetAllBidsForAuction = async (req, res) => {
  ...
  try {
    const data = await GetAllBidsForAuction(postId);
    return res.status(200).json(data);
  } catch (err) {
    return res
      .status(400)
...
export const GetAllBidsForWallet = async (req, res) => {
  ...
  try {
    const data = await GetAllBidsForWallet(name, email);
    return res.status(200).json(data);
  } catch (err) {
...
export const PostBid = async (req, res) => {
  ...
  const highestBid = QueryHighestBidForAuction(postId);
