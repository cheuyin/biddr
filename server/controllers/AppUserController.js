...
export const PostAppUser = async (req, res) => {
    const { email, username, fullName, password, location, dateOfBirth } =
        req.body;
    /*
    ...
    */
    ...
    try {
        ...
        if (location && dateOfBirth) {
            const locationAgeOfMajority = await GetLocationAgeOfMajority(
                location
            );
            ...
            const tupleExists = await GetLocationDateOfBirthIsLegalAge(
                location,
                dateOfBirth
            );
            // only create tuple if it doesn't already exist.
            if (!tupleExists[0]) {
                await CreateLocationDateOfBirthIsLegalAge(
                    location,
                    dateOfBirth,
                    isLegalAge
                );
        ...
        res.status(200).json({
            message: `Created new AppUser with email: ${email}`,
        });
    } catch (err) {
