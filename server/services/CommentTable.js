...
export const QueryCommentsByPost = async (postId) => {
    try {
        const commentsOnPost = await query("SELECT * FROM Comment WHERE postId = $1 ORDER BY timeSent DESC", [
            postId
        ]);
...
export const CreateComment = async (email, postId, text, timeSent) => {
    try {
        return await query("INSERT INTO Comment(email, postId, text, timeSent) VALUES ($1, $2, $3, $4)", 
        [
            email,
            postId,
            text,
...
export const UpdateComment = async (commentId, text, timeSent) => {
    try {
        return await query("UPDATE Comment SET text = $1, timeSent = $2 WHERE commentId = $3",
        [
            text,
            timeSent,
            commentId
